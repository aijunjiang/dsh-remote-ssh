window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function parseRouteFromCwd(cwd) {
			...
			if (cwd.includes(prefix)) {
				...
				const after = cwd.slice(idx + 15);
				const sep = after.indexOf("/");
				if (sep > 0) return {
					connectionId: after.slice(0, sep),
					path: after.slice(sep)
				};
				return null;
			}
			return null;
		}
		function RemoteFileBrowser({ rpc, scope }) {
			const [connections, setConnections] = (0, react.useState)([]);
			const [selectedId, setSelectedId] = (0, react.useState)(null);
			const [home, setHome] = (0, react.useState)("/");
			const [loading, setLoading] = (0, react.useState)(true);
			const [error, setError] = (0, react.useState)(null);
			const [autoRoute, setAutoRoute] = (0, react.useState)(null);
			const lastSessionId = (0, react.useRef)(void 0);
			(0, react.useEffect)(() => {
				...
				} else if (lastSessionId.current !== void 0 && lastSessionId.current !== scope.sessionId) {
					setAutoRoute(null);
					setSelectedId(null);
					lastSessionId.current = scope.sessionId;
				}
			...
			(0, react.useEffect)(() => {
				...
				rpc("connections.list").then((result) => {
					...
					if (result.ok) {
						...
						setConnections(list);
						if (list.length === 1 && autoRoute === null) setSelectedId(list[0].id);
			...
			if (autoRoute !== null && selectedId === autoRoute.connectionId) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(RemoteFileTree, {
				rpc,
				connectionId: autoRoute.connectionId,
				home: autoRoute.path
			});
		...
		function apply(ctx) {
			...
			if (sidebar !== void 0) ctx.effect(() => {
				...
				const disposeTab = sidebar.registerTab({
					...
					title: "Remote Files",
					component: (props) => RemoteFileBrowser({
						rpc,
						scope: props.scope
					})
				});
